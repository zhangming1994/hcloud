...
// 用户登录检验
func CheckLogin(username string, password string) (userInfo User, err error) {
	userInfo = GetUserByUname(username)
	if userInfo.Id == 0 {
		return userInfo, errors.New(" user is not exist")
	}
	passwordEncode := tool.EncodeUserPwd(password)
	if userInfo.Password != passwordEncode {
		return userInfo, errors.New("password is wrong ")
	}
	return userInfo, nil
}

// 用户登录检验(改) 用户名或工号都可登录
func Login(username string, password string) (userInfo User, err error) {
	userInfo = GetUserByUnameOrNick(username)
	if userInfo.Id == 0 {
		return userInfo, errors.New(" user is not exist")
	}
	passwordEncode := tool.EncodeUserPwd(password)
